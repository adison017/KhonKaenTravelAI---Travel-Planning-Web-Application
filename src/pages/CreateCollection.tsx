...
import { fetchWeatherForecast } from "@/lib/weatherApi"; // Import the weather API function
...
const CreateCollection = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    let weatherData = [];
    try {
      weatherData = await fetchWeatherForecast(formData.startDate, formData.endDate);
    } catch (error: any) {
      ...
      if (error.message === 'RATE_LIMIT_EXCEEDED') {
        toast({
          title: "ข้อมูลสภาพอากาศชั่วคราวไม่พร้อมใช้งาน",
          description: "ระบบมีการใช้งานสูงเกินไป (คุณยังสามารถสร้างทริปได้ตามปกติ)",
          variant: "default"
        });
      } else {
        toast({
          title: "ไม่สามารถโหลดข้อมูลสภาพอากาศ",
          description: "ระบบจะดำเนินการต่อโดยไม่มีข้อมูลสภาพอากาศ (คุณยังสามารถสร้างทริปได้ตามปกติ)",
          variant: "destructive"
        });
      }
      
      // Continue with empty weather data in case of any error
      weatherData = [];
    }
    ...
    const newCollection = {
      ...
      budget: budget,
      weatherData: weatherData, // Use the fetched or empty weather data
      plans: [
        {
          day: 1,
          startLocation: "",
